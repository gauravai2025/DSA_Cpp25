...
// Function to check if the binary tree satisfies the min heap property
bool isminheap(struct Node* tree) {
    if (tree == nullptr)
        return true;

    // Leaf node is automatically a min-heap
    ...
    bool left = isminheap(tree->left);
    bool right = isminheap(tree->right);

    return left && right;
}

// Function to check if the binary tree is a min-heap
bool isMinHeap(struct Node* tree) {
    int count = count_node(tree);

    // Check if it's both CBT and satisfies min-heap property
    if (iscbt(tree, 0, count) && isminheap(tree))
        return true;
    else
        return false;
}
