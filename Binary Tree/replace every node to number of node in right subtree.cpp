        ...
        void replaceWithrightSubtreeCount(TreeNode* node) {
            if (node == nullptr) return;

            // Count nodes in the left subtree
            int leftCount = countNodes(node->right);

            // Replace the current node's value with the count of left subtree nodes
            node->val = leftCount;

            // Recursively process the left and right subtrees
            replaceWithrightSubtreeCount(node->left);
            replaceWithrightSubtreeCount(node->right);
        }
        ...
        int main() {
            ...
            replaceWithrightSubtreeCount(root);
